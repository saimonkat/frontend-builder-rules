...
Папка hacks содержит готовые решения с реализациями различных компонентов сайта без использования библиотек

### Изменения в node_modules

Иногда при использовании библиотек из node_modules невозможно переопределить какие-то вещи. Например, скрипт библиотеки, зашитый в eventListener. В таких случаях приходится напрямую переписать несколько строк в node_modules. Но вопрос в том, как сохранить изменения зависимостей для конкретного проекта, если они всегда лежат в гитгноре и устанавливаются каждым разработчиком с нуля?
...
5. Коммитим все изменения, включая package.json и новый .patch файл и обновление библиотеки попадет в репозиторий.
